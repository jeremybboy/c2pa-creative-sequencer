...
int main()
{
    ...
    if (occlusion.result.failed() || ! occlusion.audioRendered)
        return fail(5, "plain occlusion render failed: " + occlusion.result.getErrorMessage());
    WavReadback first;
    if (! readWav(occlusionFile, first) || first.reader->numChannels != 2
        || first.reader->bitsPerSample != 24)
        return fail(6, "plain export was not a reopenable stereo 24-bit WAV");
    if (! approximately(first.reader->lengthInSamples / first.reader->sampleRate, 3.0, 0.02))
        return fail(7, "plain export duration did not end at the last audible clip");
    ...
    if (silence > 0.0001 || aBefore < 0.02 || bPriority < aBefore * 1.75
        ...
        return fail(8, "timing or same-track occlusion was incorrect in the rendered audio");

    if (engine.importAudio(sourceC, 1, 1.5).failed()
        || engine.importAudio(unused, 2, 1.5).failed()
        || engine.setTrackMute(2, true).failed())
        return fail(9, "could not place multitrack or muted fixtures");
    ...
    if (engine.duplicateClip(originalA).failed())
        return fail(10, "could not duplicate repeated source fixture");
    if (engine.importAudio(deleted, 4, 0.0).failed())
        return fail(10, "could not register deleted-source fixture");
    tracks = engine.arrangementSnapshot();
    if (engine.deleteClip(tracks[4].clips.front().id).failed())
        return fail(10, "could not remove deleted-source fixture from arrangement");
    ...
    if (mixed.result.failed() || mixed.ingredients.size() != 3)
        return fail(11, "ingredient deduplication or muted-source filtering failed");
    WavReadback second;
    if (! readWav(mixedFile, second) || rmsAt(second, 1.75) < bPriority * 1.18)
        return fail(12, "different tracks did not mix in the rendered audio");

    if (engine.setTrackSolo(1, true).failed())
        return fail(12, "could not set solo for export test");
    ...
    if (solo.result.failed() || solo.ingredients.size() != 1
        ...
        return fail(12, "soloed export did not isolate its audible source");
    if (engine.setTrackSolo(1, false).failed())
        return fail(12, "could not restore solo state after export test");

    if (engine.inspectProvenance(sourceA).status != c2paseq::ProvenanceStatus::noCredentials)
        return fail(13, "plain source ingest status was incorrect");
    ...
    if (! known.c2paPresent || known.activeManifest.isEmpty())
        return fail(14, "known authenticated source manifest was not read");
    if (engine.importAudio(juce::File(C2PA_KNOWN_SIGNED_WAV), 3, 0.0).failed()
        || engine.setTrackMute(3, true).failed())
        return fail(14, "known authenticated source could not be imported");
    tracks = engine.arrangementSnapshot();
    if (tracks[3].clips.empty() || ! tracks[3].clips.front().provenance.c2paPresent)
        return fail(14, "imported source provenance was not stored in project state");

    if (juce::SystemStats::getEnvironmentVariable(
            c2paseq::ConformanceTestSigningProvider::environmentVariable, {}).isNotEmpty())
    {
        ...
        if (signedResult.result.failed() || ! signedResult.credentialsAttached
            ...
            return fail(15, "signed export failed: " + signedResult.result.getErrorMessage());
        WavReadback signedAudio;
        if (! readWav(signedFile, signedAudio))
            return fail(16, "signed output was no longer a normal playable WAV");
        ...
        if (example.isNotEmpty() && ! signedFile.copyFileTo(juce::File(example)))
            return fail(20, "could not preserve requested example signed WAV");
    }

    std::cout << "export pipeline: stereo 24-bit render, timing, duration, same-track "
                 "occlusion, multitrack mix, ingredient filtering, and C2PA ingest passed";
    if (juce::SystemStats::getEnvironmentVariable(
            c2paseq::ConformanceTestSigningProvider::environmentVariable, {}).isNotEmpty())
        std::cout << "; signing, embedded ingredients, reopen, and tamper detection passed";
    std::cout << '\n';
    return 0;
}
